...
/*
    ...
    实现步骤：
        ...
        3.使用网络字节输出流OutputStream中的write方法，给服务器发送数据
        4.使用Socket对象中的方法getInputStream获取网络字节输入流InputStream对象
        5.使用网络字节输入流InputStream中的read方法，读取服务器回写的数据
        6.释放资源（Socket）

    注意：
        1.客户端和服务端进行交互，必须使用Socket中提供的网络流，不能使用自己创建的流对象
        2.当我们创建客户端对象Socket的时候，就会去请求服务器，和服务器经过3次握手建立连接通路
            这时如果服务器没有启动，那么就会抛出异常 java.net.ConnectException: Connection refused (Connection refused)连接异常
